...
1. ✅ TypeScript-based token generation at build time (config runs as Node.js code)
2. ✅ Musical ratio computation (functions execute in config)
3. ✅ Rhythm-based spacing (programmatic generation from base grid)
4. ✅ Polar angle tokens (rotation values as computed sets)
5. ✅ 3-layer architecture (primitive → semantic → component)
6. ✅ Theme-aware tokens (`{ base: ..., _dark: ... }` syntax)
7. ✅ Zero runtime cost (all computation at build time)
...
**Five layers (precedence high→low)**:
```css
...
// → "bg_accent-primary p_inset-normal hover:bg_accent-secondary"
```
...
Characteristics:
- ❌ No responsive in variants
- ❌ No JIT (generates all)
- ✅ Colocated (component file)
- ✅ Atomic classes (utilities layer)
- ✅ `.raw()` for composition
...
├─ colors.neo.primary: #f582ae
├─ spacing.rhythm1: 0.5rem
└─ borderWidths.brutal: 0.7rem

Layer 2: Semantic Tokens (Context-Aware)
├─ surface.bg: {_light: neo.bg, _dark: neo.fg}
├─ text.inverted: {surface.bg}
├─ accent.primary: {neo.primary}
└─ proximity.tight: {rhythm1}  ← Gestalt semantics

Layer 3: Components (Composition)
└─ Button uses accent.primary, text.inverted, proximity.tight
```
    ...
    extend: {
      ...
      tokens: defineTokens({
        ...
        fontSizes: {
          ...
          // → type0: 1rem, type1: 1.2rem, type2: 1.44rem...
        },

        spacing: {
          ...generateRhythmScale(8, [1, 2, 3, 4, 6, 8, 12, 16, 24])
          // → rhythm1: 0.5rem (8px), rhythm2: 1rem (16px)...
        },

        rotations: {
          ...generateAngleTokens()
          // → angle0: 0deg, angle45: 45deg...
        },
...
semanticTokens: {
  ...
  sizes: {
    sm: { value: '{fontSizes.type1}' }  // Alias old → new
  }
}
...
```typescript
❌ rhythm1: '0.5rem'
✅ rhythm1: { value: '0.5rem' }
```

**2. Semantic references MUST use braces**
```typescript
❌ value: 'colors.neo.primary'
✅ value: '{colors.neo.primary}'
```

**3. CVA recipes don't support responsive variants**
```typescript
❌ variants: { size: { md: { fontSize: { base: 'sm', md: 'lg' } } } }
✅ base: { fontSize: { base: 'sm', md: 'lg' } }  // Responsive in base only
```
...
```typescript
❌ value: { base: 'red', _hover: 'blue' }  // Not supported
✅ Use _hover in component cva instead
```
...
```typescript
❌ tokens: { shadows: { brutal: { value: '0.7rem 0.7rem 0 {colors.neo.fg}' } } }
✅ semanticTokens: { shadows: { brutal: { value: '0.7rem 0.7rem 0 {colors.surface.fg}' } } }
```

**Don't**: Component-specific namespaces in semantic tokens
```typescript
❌ semanticTokens: { colors: { button: { primary: ... } } }
✅ semanticTokens: { colors: { accent: { primary: ... } } }
```

**Don't**: Hardcoded values in components
```typescript
❌ cva({ base: { bg: '#f582ae', p: '1rem' } })
✅ cva({ base: { bg: 'accent.primary', p: 'inset.normal' } })
```
...
Rule: Primitive → Semantic → Component (no skipping). See Part 2.3.

### 3. Layers - How do reset/base/token/recipe/utilities cascade?

**Precedence (high→low)**: utilities > recipes > tokens > base > reset
...
- ✅ CVA recipes (not config recipes)
- ✅ Token namespace flattening
- ✅ Semantic-only spacing
- 🎯 Add ratio generation (next step)
...
| Golden Ratio | φ | 1.618 | Organic, harmonious |
...
- ✅ Complete Panda primitive reference
- ✅ Token architecture deep dive
- ✅ Recipe & pattern system
- ✅ Ratio-based implementation strategy
- ✅ Migration strategy with rollback plan
- ✅ Edge cases, gotchas, anti-patterns
- ✅ Complete code examples
- ✅ Comprehensive documentation references
- ✅ Development workflows & tooling (Part 9)
